...
class TestLoadFlowConfig:
    ...
    def test_populates_tasks(self, loaded_flow):
        """Loading config creates 9 tasks in the store (including 1 conditional)."""
        assert len(store.tasks) == 9
    ...
    def test_task_ids_match_config(self, loaded_flow):
        """All 9 expected task IDs are present in the store."""
        expected_ids = {
            "personal_details", "iq_test", "retake_iq",
