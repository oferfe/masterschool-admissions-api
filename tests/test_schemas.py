...
class TestTask:
    def test_required_fields(self):
        ...
        task = Task(id="t1", step_id="s1", name="Test Task", order=1, pass_condition="always")
        assert task.id == "t1"
    ...
    def test_defaults(self):
        """Optional fields default to conditional=False, unlock_when=None."""
        task = Task(id="t1", step_id="s1", name="Test Task", order=1, pass_condition="always")
        assert task.conditional is False
        assert task.unlock_when is None

    def test_missing_required_field_raises(self):
        """Omitting step_id (required) raises ValidationError."""
        with pytest.raises(ValidationError):
            Task(id="t1", name="Test Task", order=1, pass_condition="always")

class TestFlowStep:
    def test_required_fields(self):
        """FlowStep accepts required fields and stores them correctly."""
        task = Task(id="t1", step_id="s1", name="Task", order=1, pass_condition="always")
        step = FlowStep(id="s1", name="Step One", order=1, tasks=[task])
        assert step.id == "s1"
        assert step.name == "Step One"
        assert step.order == 1

    def test_empty_tasks_raises(self):
        """A FlowStep with no tasks raises ValidationError."""
        with pytest.raises(ValidationError):
            FlowStep(id="s1", name="Step One", order=1, tasks=[])

    def test_no_tasks_provided_raises(self):
        """Omitting tasks entirely raises ValidationError (min_length=1)."""
        with pytest.raises(ValidationError):
            FlowStep(id="s1", name="Step One", order=1)
