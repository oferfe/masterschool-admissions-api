...
class TestCompleteTaskHappyPaths:
    ...
    def test_already_passed(self, loaded_flow):
...
class TestCompleteTaskRejection:
    def test_low_score_rejects_user(self, loaded_flow):
        """IQ test with score <= 75 (outside second-chance range) rejects the user."""
        user_id = create_user("a@b.com")
        complete_task(user_id, "personal_details", {})
...
class TestTryUnlockConditionalTasks:
    def test_conditional_task_unlocked_by_matching_payload(self, loaded_flow):
        """A score in 60-75 unlocks the retake_iq conditional task."""
        user_id = create_user("a@b.com")
        assert (user_id, "retake_iq") not in store.user_task_statuses
        result = _try_unlock_conditional_tasks(user_id, {"score": 70})
        assert result is True
        assert (user_id, "retake_iq") in store.user_task_statuses

    def test_no_unlock_when_condition_not_met(self, loaded_flow):
        """A score outside 60-75 does not unlock retake_iq."""
        user_id = create_user("a@b.com")
        count_before = len(store.user_task_statuses)
        result = _try_unlock_conditional_tasks(user_id, {"score": 50})
